...
bsz        = int(sys.argv[2])
dim        = int(sys.argv[3])
n_heads    = int(sys.argv[4])
n_kv_heads = int(sys.argv[5])
head_dim   = int(sys.argv[6])
...
params = (
    f"P:{P},bsz:{bsz},dim_p_pe:{dim_p_pe},"
...
ARGS = f"--arch=wse3 {fabric} -o out --memcpy {channels} --params={params}"

app_dir = tempfile.mkdtemp(prefix="decode_hw_compile_")
try:
