...
with SdkCompiler(disable_version_check=True) as compiler:
    ...
    artifact_id = compiler.compile(
        app_path=".",
        csl_main="layout.csl",
        options="--arch=wse3 --fabric-dims=762,1172 --fabric-offsets=4,1 "
                "--params=P:180,Mt:12,Kt:12,Nt:12 --memcpy --channels=1 -o out",
        out_path=out_path
    )

    # Write the artifact_id to a JSON file (use absolute path for robustness)
