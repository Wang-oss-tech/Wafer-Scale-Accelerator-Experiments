...
from cerebras.sdk.sdk_utils import input_array_to_u32            # pylint: disable=no-name-in-module
...
A = np.random.rand(matrix_rows, matrix_cols).astype(np.float16)
X = np.random.rand(matrix_cols).astype(np.float16)
B = np.random.rand(matrix_rows).astype(np.float16)

# Compute expected result in f32 to match on-chip f32 reduction
y_expected = (A.astype(np.float32) @ X.astype(np.float32)) + B.astype(np.float32)

# Specify path to ELF files, set up runner
runner = SdkRuntime(args.name, cmaddr=args.cmaddr)

memcpy_dtype_16 = MemcpyDataType.MEMCPY_16BIT  # for f16 arrays (A, x, b)
memcpy_dtype_32 = MemcpyDataType.MEMCPY_32BIT  # for f32 arrays (y)
...
# Column-major local tile distribution: A[:,j] contiguous at A_tile[j*Mt..]
# reshape → (h, Mt, w, Nt), transpose → (h, w, Nt, Mt), flatten each tile col-major
A1 = A.reshape(kernel_rows, per_pe_rows, kernel_cols, per_pe_cols)
A2 = A1.transpose(0, 2, 3, 1)  # (h, w, Nt, Mt): columns of local tile first
data = A2.reshape(kernel_rows, kernel_cols, per_pe_rows * per_pe_cols).ravel()

# Copy flattened A array onto PEs (f16 packed as u32)
A_u32 = input_array_to_u32(data, 1, 1)
...
x_u32 = input_array_to_u32(X, 1, 1)
runner.memcpy_h2d(symbol_x, x_u32, 0, 0, 1, 1, matrix_cols,
                  streaming=False, data_type=memcpy_dtype_16, nonblock=False, order=memcpy_order)
b_u32 = input_array_to_u32(B, 1, 1)
runner.memcpy_h2d(symbol_b, b_u32, 0, 0, 1, 1, matrix_rows,
                  streaming=False, data_type=memcpy_dtype_16, nonblock=False, order=memcpy_order)
...
# y is f32 (result of reduce_fadds)
